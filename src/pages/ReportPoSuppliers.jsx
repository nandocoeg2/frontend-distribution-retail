...
const ReportPoSuppliers = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!response?.success) throw new Error(response?.error?.message || 'Gagal memuat data report.');
      ...
      console.error('Failed to fetch report PO suppliers:', err);
  ...
  const handlePageChange = useCallback((p) => {
    setPage(p);
    setPaginationInfo((prev) => ({ ...prev, currentPage: p }));
  }, []);

  const handleLimitChange = useCallback((l) => {
    setLimit(l);
    setPaginationInfo((prev) => ({ ...prev, itemsPerPage: l }));
    setPage(1);
  }, []);
    ...
    <Card padding='md' className='shadow-sm'>
      ...
      {error && (
        <div className='mt-2 flex items-center justify-between rounded border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-800'>
          ...
          <button type='button' onClick={handleRetryFetch} className='rounded bg-red-600 px-2 py-1 text-xs font-medium text-white hover:bg-red-700'>
            Muat ulang
          </button>
        </div>
      )}
