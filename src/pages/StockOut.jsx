...
const StockOut = () => {
  ...
  // Set default filter type to STOCK_OUT on mount
  useEffect(() => {
    handleFiltersChange({ type: 'STOCK_OUT' });
  }, [handleFiltersChange]);
  ...
  const handleStockOutSubmit = async (payload) => {
    try {
      await createStockOutMovement(payload);
  ...
  return (
    <div className='min-h-screen bg-gray-50 py-6 px-4 sm:px-6 lg:px-8 space-y-6'>
      ...
      <div className='bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex flex-col md:flex-row items-start md:items-center justify-between gap-4'>
        <div>
          ...
          <p className='text-xs text-gray-500 mt-1'>
            Kelola dan pantau seluruh transaksi pengeluaran stok barang (Stock Out).
          </p>
        </div>
        ...
        <div className='flex items-center gap-3 w-full md:w-auto justify-end'>
          ...
          <button
            ...
            className='inline-flex items-center gap-1.5 rounded-lg bg-amber-600 px-4 py-2 text-xs font-semibold text-white shadow-sm hover:bg-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-600 focus:ring-offset-2'
          >
      ...
      {/* Date Range & Reload Controls Header (Matching Image 2 UI) */}
      <div className='bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex flex-wrap items-center justify-between gap-4'>
        <div className='flex flex-wrap items-center gap-4'>
          <div className='flex items-center gap-2'>
            ...
            <input
              ...
              className='rounded-md border border-gray-300 px-3 py-1.5 text-xs text-gray-800 focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500'
            />
          </div>
        ...
        <div className='w-full sm:w-64'>
          <input
            ...
            placeholder='Cari nomor PO / customer / barang...'
            className='w-full rounded-md border border-gray-300 px-3 py-1.5 text-xs text-gray-800 focus:border-amber-500 focus:outline-none'
          />
        </div>
      </div>
