...
const BulkUploadGroupCustomer = ({ onClose, onSuccess }) => {
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      await groupCustomerService.downloadBulkTemplate();
  ...
  const handleUpload = async () => {
    ...
    try {
      setUploading(true);
      const response = await groupCustomerService.uploadBulkGroupCustomer(selectedFile);
      ...
      if (response.success && response.data) {
        ...
        setUploadStatus({
          bulkId: response.data.bulkId,
          totalFiles: response.data.totalFiles,
          status: 'PROCESSING BULK GROUP CUSTOMER'
        });
        ...
        <div className="flex items-start space-x-3">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-sm text-blue-700">
              Download template Excel terlebih dahulu, isi data group customer sesuai format yang tersedia.
            </p>
            <button
              type="button"
...
export default BulkUploadGroupCustomer;
