...
const BulkUploadTermOfPayment = ({ onClose }) => {
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      await termOfPaymentService.downloadBulkTemplate();
  ...
  const pollUploadStatus = async (bulkId) => {
    try {
      const response = await termOfPaymentService.getBulkUploadStatus(bulkId);
      if (response.success && response.data) {
        ...
        if (status === 'COMPLETED BULK TERM OF PAYMENT' || status === 'FAILED BULK TERM OF PAYMENT') {
          if (pollingInterval) {
            clearInterval(pollingInterval);
            setPollingInterval(null);
          }

          if (status === 'COMPLETED BULK TERM OF PAYMENT') {
  ...
  const handleUpload = async () => {
    ...
    try {
      setUploading(true);
      const response = await termOfPaymentService.uploadBulkTermOfPayment(selectedFile);
      ...
      if (response.success && response.data) {
        ...
        setUploadStatus({
          ...
          status: 'PROCESSING BULK TERM OF PAYMENT'
        });
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case 'COMPLETED BULK TERM OF PAYMENT':
        return 'text-green-600 bg-green-50';
      case 'FAILED BULK TERM OF PAYMENT':
        return 'text-red-600 bg-red-50';
      case 'PROCESSING BULK TERM OF PAYMENT':
        return 'text-blue-600 bg-blue-50';
      case 'PENDING BULK TERM OF PAYMENT':
        ...
        <div className="flex items-start space-x-3">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-sm text-blue-700">
              Download template Excel terlebih dahulu, isi data term of payment sesuai format yang tersedia.
            </p>
            <button
              type="button"
...
export default BulkUploadTermOfPayment;
