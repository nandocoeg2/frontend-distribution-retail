...
const BulkUploadCustomer = ({ onClose }) => {
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      await customerService.downloadBulkTemplate();
  ...
  const handleUpload = async () => {
    ...
    try {
      setUploading(true);
      const response = await customerService.uploadBulkCustomer(selectedFile);
      ...
      if (response.success && response.data) {
        ...
        setUploadStatus({
          ...
          status: 'PROCESSING BULK CUSTOMER'
        });
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case 'COMPLETED BULK CUSTOMER':
        return 'text-green-600 bg-green-50';
      case 'FAILED BULK CUSTOMER':
        return 'text-red-600 bg-red-50';
      case 'PROCESSING BULK CUSTOMER':
        return 'text-blue-600 bg-blue-50';
      case 'PENDING BULK CUSTOMER':
        ...
        <div className="flex items-start space-x-3">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-sm text-blue-700">
              Download template Excel terlebih dahulu, isi data customer sesuai format yang tersedia.
            </p>
            <button
              type="button"
