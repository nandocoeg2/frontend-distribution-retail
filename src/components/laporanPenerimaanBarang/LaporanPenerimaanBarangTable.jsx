...
const LaporanPenerimaanBarangTable = ({
  ...
  const allSelected =
    hasSelectableRows && selectableIds.every((id) => selectedIds.includes(id));
        ...
        <div className='flex items-center justify-between p-4 border border-blue-200 rounded-lg bg-blue-50'>
          ...
          <div className='flex items-center gap-2'>
            <button
              ...
              className='inline-flex items-center px-4 py-2 text-sm font-medium text-white transition-colors bg-blue-600 rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:cursor-not-allowed disabled:opacity-60'
            >
              {isProcessing ? (
                <svg
            ...
            <button
              ...
              disabled={isProcessing || isCompleting}
              className='inline-flex items-center px-4 py-2 text-sm font-medium text-white transition-colors bg-green-600 rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 disabled:cursor-not-allowed disabled:opacity-60'
            >
              {isCompleting ? (
                <svg
                  className='-ml-0.5 mr-2 h-4 w-4 animate-spin text-white'
            ...
            <tr>
              <th className='px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase'>
                <input
                  ...
                  onChange={(e) =>
                    onSelectAllReports && onSelectAllReports(e.target.checked)
                  }
                  disabled={
                    selectionDisabled ||
                    isProcessing ||
          ...
          <tbody className='bg-white divide-y divide-gray-200'>
            {data.length === 0 ? (
              ...
              data.map((report, index) => {
                ...
                const poNumber =
                  report?.purchaseOrder?.po_number ||
                  report?.purchaseOrderId ||
                  '-';
                const customerName =
                  report?.customer?.namaCustomer || report?.customerId || '-';
                const terminName =
                  report?.termOfPayment?.kode_top ||
                  report?.termin_bayar ||
                  '-';
                const statusName =
                  ...
                  report?.statusId ||
                  '-';
                const fileCount = Array.isArray(report?.files)
                  ? report.files.length
                  : 0;
                ...
                return (
                  <tr
                    key={
                      reportId || report?.id || report?.purchaseOrderId || index
                    }
                  ...
                  >
                    ...
                    <td className='px-6 py-4 text-sm font-medium text-right whitespace-nowrap'>
                      <div className='flex justify-end space-x-2'>
                        ...
                        <button
                          onClick={() =>
                            onDelete && reportId && onDelete(reportId)
                          }
