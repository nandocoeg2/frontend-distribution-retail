...
const FakturPajakTableServerSide = ({
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor((row) => row.laporanPenerimaanBarang?.no_lpb || row.invoicePenagihan?.purchaseOrder?.laporanPenerimaanBarang?.no_lpb, {
        id: 'no_lpb',
        header: ({ column }) => (
          <div className="space-y-1">
        ...
        cell: (info) => {
          ...
          return (
            <div>
              ...
              {(item?.laporanPenerimaanBarang?.tanggal_po || item?.invoicePenagihan?.purchaseOrder?.laporanPenerimaanBarang?.tanggal_po) && (
                <div className="text-xs text-gray-500">
                  {formatDate(item?.laporanPenerimaanBarang?.tanggal_po || item?.invoicePenagihan?.purchaseOrder?.laporanPenerimaanBarang?.tanggal_po)}
                </div>
              )}
            </div>
          );
