...
const BulkUploadSupplier = ({ onClose }) => {
  ...
  const pollUploadStatus = async (bulkId) => {
    try {
      ...
      if (response.success && response.data) {
        ...
        if (status === 'COMPLETED BULK SUPPLIER' || status === 'FAILED BULK SUPPLIER') {
          if (pollingInterval) {
            clearInterval(pollingInterval);
            setPollingInterval(null);
          }

          if (status === 'COMPLETED BULK SUPPLIER') {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case 'COMPLETED BULK SUPPLIER':
        return 'text-green-600 bg-green-50';
      case 'FAILED BULK SUPPLIER':
        return 'text-red-600 bg-red-50';
      case 'PROCESSING BULK SUPPLIER':
        return 'text-blue-600 bg-blue-50';
      case 'PENDING BULK SUPPLIER':
