...
const CreateStockInModal = ({
  ...
  const handleFieldChange = (event) => {
    ...
    // Strip leading whitespace, limit to 255 chars (match backend max)
  ...
  const validateForm = () => {
    ...
    if (!form.supplierId.trim()) return 'Supplier wajib dipilih.';
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setFormError(err?.message || 'Gagal mencatat stock in. Coba lagi nanti.');
    } finally {
      setIsSubmitting(false);
    }
  };
    ...
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4'>
      <div className='w-full max-w-4xl overflow-hidden rounded-lg bg-white shadow-xl ring-1 ring-gray-200'>
        ...
        <div className='flex items-center justify-between border-b border-gray-200 bg-indigo-600 px-5 py-3 text-white'>
          <h2 className='text-base font-semibold'>Stock In</h2>
          <button
            type='button'
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className='max-h-[85vh] overflow-y-auto'>
          ...
          <div className='border-b border-gray-100 px-5 py-4'>
            <div className='grid gap-3 sm:grid-cols-3'>
              ...
              <div>
                <label htmlFor='supplierId' className='block text-xs font-medium text-gray-600 mb-1'>Supplier</label>
                <Autocomplete
                  label='' name='supplierId' options={supplierOptions}
                  value={form.supplierId ? String(form.supplierId) : ''}
                  onChange={handleSupplierChange} placeholder='Cari supplier...'
                  displayKey='label' valueKey='id'
