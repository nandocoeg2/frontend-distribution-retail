...
const BulkUploadRegion = ({ onClose }) => {
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      await regionService.downloadBulkTemplate();
  ...
  const pollUploadStatus = async (bulkId) => {
    try {
      const response = await regionService.getBulkUploadStatus(bulkId);
      if (response.success && response.data) {
        ...
        if (status === 'COMPLETED BULK REGION' || status === 'FAILED BULK REGION') {
          if (pollingInterval) {
            clearInterval(pollingInterval);
            setPollingInterval(null);
          }

          if (status === 'COMPLETED BULK REGION') {
  ...
  const handleUpload = async () => {
    ...
    try {
      setUploading(true);
      const response = await regionService.uploadBulkRegion(selectedFile);
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case 'COMPLETED BULK REGION':
        return 'text-green-600 bg-green-50';
      case 'FAILED BULK REGION':
        return 'text-red-600 bg-red-50';
      case 'PROCESSING BULK REGION':
        return 'text-blue-600 bg-blue-50';
      case 'PENDING BULK REGION':
        ...
        <div className="flex items-start space-x-3">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-sm text-blue-700">
              Download template Excel terlebih dahulu, isi data region sesuai format yang tersedia.
            </p>
            <button
              type="button"
...
export default BulkUploadRegion;
