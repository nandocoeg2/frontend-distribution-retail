...
const SuratJalanTableServerSide = ({
  ...
  const { data, isLoading, isFetching, error } = useSuratJalanQuery({
    page,
    limit,
    sorting,
    filters,
  ...
  const suratJalan = data?.suratJalan || [];
      ...
      columnHelper.display({
        ...
        header: ({ table }) => {
          ...
          return (
            <input
              ...
              onChange={() => onSelectAllSuratJalan(suratJalan)}
        ...
        cell: ({ row }) => (
          <input
            ...
            onChange={() => onSelectSuratJalan(row.original.id)}
            className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
          />
        ),
        enableSorting: false,
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-white border-t border-gray-200">
            <div className="flex items-center space-x-2">
              <span className="text-sm text-gray-700">
                ...
                dari <span className="font-medium">{pagination.totalItems}</span> surat jalan
              </span>
            </div>
